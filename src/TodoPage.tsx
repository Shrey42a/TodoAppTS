...
function ToDoPage() {
  ...
  const [todoList, setTodoList] = React.useState <string []>(savedTodoList);
  ...
  const addTodo = (todoTitle: string) => {
   ...
   const onTodoDelete = (todo: string) => {
    const newTodoList = todoList.filter(t => t.id !== todo.id);
     setTodoList(newTodoList);
     localStorage.setItem('todoList', JSON.stringify(newTodoList));
  }
  
  const markAsDone = (todo: string) => {
    todo.done = true;
  ...
  const markAsNotDone = (todo: string) => {
    todo.done = false;
    const newTodoList = [...todoList];
    setTodoList([...newTodoList]);
    localStorage.setItem('todoList', JSON.stringify(newTodoList));
